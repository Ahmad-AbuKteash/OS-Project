 ...
 * parent-child communication. Each child gets its own pipe to send node-arrival
 ...
 *      parent through the pipe, then sleeps to simulate travel time.
...
void runChild(int startNode, int endNode, int writeFd, Graph* graph) {
    pid_t myPid = getpid();
...
int main(int argc, char* argv[]) {
    ...
    // Creating ALL pipes before any fork ensures every child inherits only its
    // own write-end and no stale write-ends are left open in the parent.
    ...
    for (int i = 0; i < numTravelers; i++) {
        ...
        if (pid == 0) {
            // ── Child process ────────────────────────────────────────────────
            // Close ALL read-ends (children never read)
            for (int j = 0; j < numTravelers; j++)
                close(travelers[j].pipefd[0]);
            // Close write-ends of all OTHER travelers' pipes
            for (int j = 0; j < numTravelers; j++) {
                if (j != i) close(travelers[j].pipefd[1]);
            }
            int writeFd = travelers[i].pipefd[1];
            runChild(travelers[i].startNode, travelers[i].endNode, writeFd, graph);
    ...
    // Parent closes ALL write-ends after all forks are done
    for (int i = 0; i < numTravelers; i++) {
